...
En este nuevo post vamos a centrarnos en la creación de una visualización a partir de datos abiertos. Las visualizaciones de datos ofrecen una manera accesible para detectar tendencias y patrones de los datos. Son esenciales para analizar los conjuntos de datos y tomar decisiones basadas en ellos.
...
Para este análisis se han utilizado diferentes conjuntos de datos: 
...
Todos estos conjuntos de datos se encuentran disponibles en [datos.gob.es](https://datos.gob.es/). Se tratan de conjuntos de datos que recogen series temporales mensuales por Provincias o Municipios. Estos datos han sido publicados por el Servicio Público de Empleo Estatal (Ministerio de Trabajo y Economía Social).   
...
> RStudio es un entorno de desarrollo integrado para el lenguaje de programación R, dedicado al análisis estadístico y la creación de gráficos. 

Para la creación de los gráficos interactivos se han usado la herramienta de [Elastic Stack](https://www.elastic.co/es/elastic-stack?ultron=B-Stack-Trials-EMEA-S-Exact&gambit=Elasticsearch-ELK&blade=adwords-s&hulk=cpc&Device=c&thor=elastic%20stack&gclid=Cj0KCQiA0fr_BRDaARIsAABw4Evg_ld47Q6u6eE89zQ3Jl1TPZqX1UG9xebZfyUBpo6Z7QX_fO4txgMaAuuAEALw_wcB). 
...
Instalamos y cargamos las librerías necesarias para el tratamiento y análisis de estos datos. 
...
Cargamos los conjuntos de datos en RStudio desde su ubicación en la página web para poder trabajar con ellos.      
...
class(Contratos_2013_2s)
```
Una vez que tenemos todos los datasets que nos interesan los unimos en un único conjunto de datos. 
...
```{r Unir datasets 2, warning = FALSE}
#Dataset de gasto en prestaciones por desempleo en 2013
...
```{r Unir datasets 3, warning = FALSE}
#Dataset de nuevos contratos a demandates de empleo en 2013
...
Cuando tenemos el dataset que nos interesa, debemos evaluar el tipo de variables que presenta y si fuera necesario transformarlas, ya que en algunas ocasiones RStudio no interpreta el "tipo" de manera correcta. Por ejemplo, en estos tres dataset tenemos una variable que hace referencia a la fecha, RStudio la interpreta como "numeric", por tanto debemos transformarla para que esa variable sea tomada como una fecha.   
...
#Transformamos una variable que RStudio ha interpretado de tipo "character", pero es de tipo "numeric"
